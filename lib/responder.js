...
var responder = (function() {
    ...
    function json(res, json) {
      responder.respond(res, JSON.stringify(json));
    }
    
    function serveTemplate(res, name, user) {
      template.serve(name, function(html) {
		    var body = jade.render(html, { locals: user.scope }),
            ...
            template.serve('layout', function(layout) {
              responder.respond(res, jade.render(layout, {
                locals: {
                  body: body,
                  title: title,
	...
	function respond404(res) {
		responder.respond(res,'<h1>404 Not Found</h1>', 'text/html', 404);
	}

	function respond500(res, error){
		log('Responding 500 - ' + error);
		responder.respond(res, '<h1>500 Internal Server Error</h1>', 'text/html', 500);
	}
	...
	function serveFile(req, res, file) {
		...
		fs.readFile(file, encoding, function(error, data) {
			...
			if(error) {
				...
				return responder.respond404(res);
			}

			log('Serving file - ' + file);
			responder.respond(res, data, contentType, 200, encoding);
		});
